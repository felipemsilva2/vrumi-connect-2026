-- =====================================================
-- GAMIFICATION SCHEMA: Streaks + XP System
...
-- 2. User XP Table (accumulated points)
...
-- 3. XP History (for tracking what earned XP)
CREATE TABLE IF NOT EXISTS xp_history (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
    xp_amount INTEGER NOT NULL,
    action_type TEXT NOT NULL, -- 'flashcard', 'quiz', 'streak', 'challenge', 'note'
    description TEXT,
    created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE POLICY "Users can manage own daily goals" ON user_daily_goals
