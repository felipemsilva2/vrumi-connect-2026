...
const InstructorCard3D = ({ instructor, formatPrice, index }: { instructor: Instructor; formatPrice: (p: number) => string; index: number }) => {
  ...
  return (
    <Link to={`/connect/instrutor/${instructor.id}`}>
      <motion.div
        className="group relative overflow-hidden rounded-2xl bg-white border border-connect/10 cursor-pointer transform-gpu shadow-lg hover:shadow-2xl transition-shadow duration-500"
        initial={{ opacity: 0, y: 50, rotateX: -15, scale: 0.95 }}
        whileInView={{ opacity: 1, y: 0, rotateX: 0, scale: 1 }}
        viewport={{ once: true, margin: "-50px" }}
        transition={{ duration: 0.6, delay: index * 0.08, type: "spring", stiffness: 80, damping: 15 }}
      ...
      >
        ...
        <motion.div
          className="absolute inset-0 bg-gradient-to-br from-connect/5 via-transparent to-connect-accent/5 opacity-0 z-10 pointer-events-none"
        ...
        {/* Content */}
        <CardContent className="p-5 relative" style={{ transform: "translateZ(20px)" }}>
          ...
          <div className="flex items-center gap-2 mb-4">
            {instructor.categories.map((cat) => (
              <Badge 
                key={cat} 
                variant="secondary" 
                className="bg-connect/10 text-connect hover:bg-connect/20 font-medium"
              >
                {cat}
              </Badge>
            ))}
          ...
          <div className="flex items-center justify-between pt-3 border-t border-connect/10">
            ...
            <motion.div
              className="flex items-center text-connect-accent font-semibold text-sm"
...
const TrustCard3D = ({ icon: Icon, title, description, index }: { icon: any; title: string; description: string; index: number }) => {
  const [hovered, setHovered] = useState(false);
  const [mousePos, setMousePos] = useState({ x: 0, y: 0 });
  ...
  return (
    <motion.div
      className="relative text-center p-8 rounded-2xl bg-white border border-connect/10 shadow-lg transform-gpu cursor-default"
      ...
      animate={{
        ...
        rotateY: mousePos.x,
        boxShadow: hovered 
          ? "0 25px 50px -12px rgba(15, 42, 68, 0.25)" 
          : "0 10px 15px -3px rgba(0, 0, 0, 0.1)",
      }}
      style={{ transformStyle: "preserve-3d", perspective: "1000px" }}
    >
      <motion.div 
        className="w-16 h-16 bg-gradient-to-br from-connect to-connect-light rounded-2xl flex items-center justify-center mx-auto mb-5 shadow-lg"
        animate={{ 
          scale: hovered ? 1.1 : 1,
          rotateZ: hovered ? 5 : 0,
        }}
      ...
      <h3 className="font-bold text-connect text-lg mb-3" style={{ transform: "translateZ(20px)" }}>
        {title}
      </h3>
      ...
      <div className="min-h-screen bg-connect-surface">
        ...
        <section className="relative bg-gradient-to-br from-connect via-connect to-connect-dark text-white overflow-hidden">
          ...
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            <motion.div
              className="absolute top-20 right-20 w-96 h-96 rounded-full bg-connect-accent/10 blur-3xl"
              animate={{ 
                scale: [1, 1.2, 1],
                opacity: [0.3, 0.5, 0.3],
              }}
          ...
          <div className="container mx-auto px-4 py-20 md:py-28 relative z-10">
            ...
            >
              ...
              {/* Search Form */}
              <motion.div 
                className="bg-white rounded-2xl p-4 md:p-6 max-w-4xl mx-auto shadow-2xl"
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  <Input
                    ...
                    className="h-14 text-connect border-connect/20 focus:ring-connect"
                  />
                  ...
                  <Button 
                    ...
                    className="h-14 bg-connect hover:bg-connect-light text-white font-semibold shadow-lg hover:shadow-xl transition-all"
                  >
        ...
        <section className="container mx-auto px-4 py-16">
          ...
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {[...Array(8)].map((_, i) => (
                <div key={i} className="rounded-2xl overflow-hidden border border-connect/10 bg-white">
